...
split_first(std::string_view str, char delimiter) {
...
inline std::expected<request, error_code>
parse_http1_request(std::string_view data) {
  request req;
  ...
  while (pos < data.size()) {
    ...
    // header 名称归一化为小写
    std::string norm_name = std::string(trim(name));
    std::transform(norm_name.begin(), norm_name.end(), norm_name.begin(),
...
parse_http1_response(std::string_view data) {
  response resp;
  ...
  auto result =
      std::from_chars(status_str.data(), status_str.data() + status_str.size(),
                      resp.status_code);
  if (result.ec != std::errc{}) {
...
inline std::expected<request, error_code>
parser::parse_request(std::string_view data) {
  return detail::parse_http1_request(data);
}

inline std::expected<response, error_code>
parser::parse_response(std::string_view data) {
  return detail::parse_http1_response(data);
}

inline std::expected<size_t, error_code>
parser::parse_request_incremental(std::span<const char> data, request &req) {
...
inline std::expected<size_t, error_code>
parser::parse_response_incremental(std::span<const char> data, response &resp) {
  std::string_view data_view{data.data(), data.size()};
...
inline void parser::reset() noexcept { reset_state(); }
